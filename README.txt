...
The program tracks which sentences have been played, and how often they have been answered correctly, to bias selection of sentences to those that have been gotten wrong more often (though any sentence in a unit can be chosen at any time).  More details on how this is done can be found below.
...
NOTE: While there are a few sample units included, they were written by a non-Finnish speaker and should not be trusted to be correct.  Make your own sentences!  Learn!  Contributions would be welcome, too!
...
If you want to create your own units, or edit the sentences / verbs that are already in the program, open up the Excel spreadsheet file in the "data" directory.  Make any changes you want to that file (being careful to not change the formatting of the spreadsheet - see instructions in the sheet), save it with the same filename, and then "update" the database via the database menu in the program. This will add new sentences/verbs/units to the database as well as updating all sentences in the database with the information in the file (dumbly - it does not check if sentences have changed, it just updates everything), without deleting your user data.  When you update the database, keep an eye out for warnings after importing - the program should handle most cases of errors without causing crashes, but may not do what you intended if things have been mis-formatted in the spreadsheet (the importer is not flexible at all - all columns must be maintained as in the template).  Errors print out after updating (scroll up if you miss them).

The development GoogleSheets version of the sentence spreadsheet can be found here: https://docs.google.com/spreadsheets/d/1rvbic9S8lc1iQwkLT6Nru_jIY32eXl3b39gwMAlU_Wg/edit?usp=sharing

INSTALLATION:

Place the files in a directory on your computer, and run the opinsuomea.py file using Python 3.

If you don't have python on your computer:
1) Install Python 3 (programmed using 3.10, but probably fine on earlier versions of 3) - https://www.python.org/downloads/
2) Open a terminal / command line window
3) Run the command "pip3 install openpyxl" or "pip install openpyxl" to install a module the program requires for reading the Excel spreadsheet.

After placing the program files in a directory
1) Open a terminal / command line window (Command prompt on Windows, Terminal on a Mac, or whatever you want)
2) Navigate to the directory this script is in.
3) Run the command "python3 opinsuomea.py"
